...
static void a2dp_demo_timer_start(a2dp_media_sending_context_t * context){
    context->max_media_payload_size = 0x290; //btstack_min(a2dp_max_media_payload_size(context->a2dp_cid, context->local_seid), SBC_STORAGE_SIZE);
...
static void a2dp_source_packet_handler(uint8_t packet_type, uint16_t channel, uint8_t *packet, uint16_t size){
    ...
    switch (hci_event_a2dp_meta_get_subevent_code(packet)){
        ...
        case A2DP_SUBEVENT_SIGNALING_MEDIA_CODEC_SBC_CONFIGURATION:
            ...
            avdtp_source_open_stream(media_tracker.a2dp_cid, media_tracker.local_seid, media_tracker.remote_seid);
            break;
...
void BT_A2DPSource_connect(bd_addr_t addrress) {
    //avdtp_source_connect(addr, &media_tracker.avdtp_cid);
    printf("Connecting to nothing device.\n");
    uint8_t addr[] = {0x2C, 0xBE, 0xEB, 0x76, 0x42, 0x8F};

    connection_requested = true;

    avdtp_source_connect(addr, &media_tracker.a2dp_cid);
}
